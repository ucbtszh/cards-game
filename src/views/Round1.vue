...
export default {
  ...
  methods: {
    ...
    saveBlock1: function() {
      // SAVE RESPONSES FROM BLOCK 1 FOR ONE OF NEXT BLOCKS
      let data = [];
      for (let i = 0; i < this.trials.length; i++) {
        data.push({
          n_red: this.trials[i].n_red,
          outcome: this.reportColour[i],
          randomPick: this.randomPickColour[i],
          lied:
            ((this.reportColour[i] == 1) &
              (this.randomPickColour[i] == "blue")) |
            ((this.reportColour[i] == -1) & (this.randomPickColour[i] == "red"))
              ? 0
              : 1,
        });
      }
